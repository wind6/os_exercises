...
>  首先要通过pic_init完成8259中断控制器的初始化,把8259配置好之后,相应的外设就能够产生中断,并被CPU所接收和处理。之后要初始化IDT，即中断描述符表的,这个中断描述符表是一个大的数组,每一个索引能代表一个中断号,当一个中断产生之后,它会有对应这个中断号,比如说时钟中断它会有对应的中断号,这个中断号会用来作为index,来查IDT描述符表,这个表里面对应index一项呢,记录了当产生中断之后,所要去应对中断服务例程的地址。最后要完成对8253时钟外设的初始化，使之每一百个tick,会产生一次中断。完成以上PIC IDT Clock这三步初始化，就可以通过STI指令使能中断。
...
> 首先要实现一个可显示字符串的bootloader需要注意完成对中断的使能，通过串口，并口，cga的中断来显示字符。之后实现可加载ELF格式文件的bootloader需要注意首先要完成实模式到保护模式的切换（实现GDT，加载到GDTR寄存器，CR0最高位置1等），然后要识别文件系统，读取ELF格式的内核文件。
